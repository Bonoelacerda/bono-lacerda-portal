...
function Toast({msg,onClose}){useEffect(()=>{const t=setTimeout(onClose,3500);return()=>clearTimeout(t)},[]);return<div className="toast">âœ“ {msg}</div>;}
...
function Dash({ clients }) {
  ...
  return (
    <div>
      ...
      {/* ÃšLTIMOS ACESSOS AO PORTAL â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
      <div className="card cp" style={{marginBottom:"1.25rem"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:".75rem"}}>
          <div className="ct" style={{margin:0}}>ðŸŸ¢ Ãšltimos Acessos ao Portal</div>
        ...
        {ldLogins
          ? <div style={{color:"var(--mu)",fontSize:".82rem",textAlign:"center",padding:"1rem"}}>A carregar acessosâ€¦</div>
          : recentLogins.length === 0
            ? <div style={{color:"var(--mu)",fontSize:".85rem",textAlign:"center",padding:"1.5rem"}}>Nenhum acesso registado ainda. Os acessos aparecerÃ£o aqui quando os clientes entrarem no portal.</div>
                ...
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:".82rem"}}>
                  <thead>
                    <tr style={{borderBottom:"2px solid var(--bo)",textAlign:"left"}}>
                      ...
                      <th style={{padding:".5rem .6rem",color:"var(--mu)",fontWeight:600,fontSize:".72rem",textTransform:"uppercase",letterSpacing:".06em"}}>Fuso/RegiÃ£o</th>
                    </tr>
                  </thead>
                  <tbody>
                    {recentLogins.map((log,i) => (
                      <tr key={log.id||i} style={{borderBottom:"1px solid var(--bo)"}}>
                        <td style={{padding:".55rem .6rem",fontWeight:600}}>{log.clients?.name || "â€”"}</td>
                        <td style={{padding:".55rem .6rem",color:"var(--mu)"}}>
                          {log.logged_in_at ? new Date(log.logged_in_at).toLocaleString("pt-BR",{day:"2-digit",month:"2-digit",year:"2-digit",hour:"2-digit",minute:"2-digit"}) : "â€”"}
                        </td>
                        <td style={{padding:".55rem .6rem"}}>
                          <span style={{background:log.device_type==="mobile"?"#dbeafe":"#f0fdf4",color:log.device_type==="mobile"?"#1d4ed8":"#16a34a",fontSize:".72rem",fontWeight:600,padding:".15rem .5rem",borderRadius:99}}>
                            {log.device_type==="mobile"?"ðŸ“± Mobile":"ðŸ’» Desktop"}
                          </span>
                        </td>
                        <td style={{padding:".55rem .6rem",color:"var(--mu)",fontSize:".78rem"}}>{log.country || "â€”"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"1.25rem",marginBottom:"1.25rem"}}>
        ...
        <div className="card cp">
          <div className="ct" style={{marginBottom:".75rem"}}>ðŸ” Busca RÃ¡pida de Cliente</div>
          <input
            value={search}
            ...
            filtered.length === 0
              ...
              : filtered.slice(0,5).map(c => (
                <div key={c.id} style={{display:"flex",alignItems:"center",gap:".75rem",padding:".6rem .75rem",borderRadius:10,border:"1px solid var(--bo)",marginBottom:".5rem",background:"var(--bg)"}}>
                  ...
                  {c.pendencias && <span title={c.pendencias} style={{fontSize:"1rem"}}>âš ï¸</span>}
                </div>
              ))
          )}
          {!search.trim() && (
        ...
        <div className="card cp">
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:".75rem"}}>
            <div className="ct" style={{margin:0}}>ðŸ“… Agenda desta semana</div>
            <a href="https://calendar.google.com/calendar" target="_blank" rel="noreferrer"
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"1.25rem",marginBottom:"1.25rem"}}>
        ...
        {/* 4. DISTRIBUIÃ‡ÃƒO DOS PROCESSOS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
        <div className="card cp">
          <div className="ct" style={{marginBottom:"1rem"}}>ðŸ“Š DistribuiÃ§Ã£o dos Processos</div>
          ...
          <div style={{marginBottom:"1rem"}}>
            ...
            {[
              ["ðŸ›ï¸ Arquivo Central do Porto", porto,  "#0f1e35"],
              ["ðŸ›ï¸ ConservatÃ³ria Reg. Centrais", crc, "#b8860b"],
          ...
          <div>
            ...
            {[
              ["âœ… Em andamento",  emAndamento, "#16a34a"],
        ...
        {/* 5. CLIENTES COM PENDÃŠNCIAS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
        <div className="card cp">
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:".75rem"}}>
            <div className="ct" style={{margin:0}}>âš ï¸ PendÃªncias Activas</div>
          ...
          <div style={{maxHeight:320,overflowY:"auto"}}>
            {comPendencias.length === 0
              ? <div style={{color:"var(--mu)",fontSize:".85rem",textAlign:"center",padding:"2rem"}}>âœ… Sem pendÃªncias activas</div>
              : comPendencias.map(c => (
...
const WA_TEMPLATES = [
  {
    id: "acesso",
    label: "ðŸ”‘ Enviar Acesso ao Portal",
    icon: "ðŸ”‘",
    msg: (c) =>
`OlÃ¡ ${c.name.split(" ")[0]}! ðŸ‘‹
...
ðŸ”— *Acesse agora:* bono-lacerda-portal.vercel.app
ðŸ”‘ *Chave de acesso:* ${c.chave_acesso || "â€”"}
...
Qualquer dÃºvida estamos disponÃ­veis! ðŸ˜Š
*Bono & Lacerda Advogados*
ðŸ“ž +351 21 793 1934`
  },
  {
    id: "atualizacao",
    label: "ðŸ“‹ ActualizaÃ§Ã£o do Processo",
    icon: "ðŸ“‹",
    msg: (c, proc) =>
`OlÃ¡ ${c.name.split(" ")[0]}! ðŸ‘‹
...
ðŸ“Œ *Estado actual:* ${proc?.status === "em_andamento" ? "Em andamento âœ…" : proc?.status === "aguardando" ? "Aguardando documentos âš ï¸" : "ConcluÃ­do ðŸŽ‰"}
ðŸ›ï¸ *Local:* ${proc?.arquivo || "IRN"}
ðŸ“… *Ãšltima atualizaÃ§Ã£o:* ${proc?.last_update ? new Date(proc.last_update).toLocaleDateString("pt-BR") : "â€”"}

Para mais detalhes aceda ao portal:
ðŸ”— bono-lacerda-portal.vercel.app
ðŸ”‘ Chave: ${c.chave_acesso || "â€”"}
...
ðŸ“ž +351 21 793 1934`
  ...
  {
    id: "pendencia",
    label: "âš ï¸ PendÃªncia Identificada",
    icon: "âš ï¸",
    msg: (c) =>
`OlÃ¡ ${c.name.split(" ")[0]}! ðŸ‘‹
...
âš ï¸ *PendÃªncia:* ${c.pendencias || "DocumentaÃ§Ã£o em falta"}
${c.observacao ? `ðŸ“ *Detalhe:* ${c.observacao}` : ""}
...
ðŸ“ž +351 21 793 1934`
  ...
  {
    id: "reuniao",
    label: "ðŸ“… Confirmar ReuniÃ£o",
    icon: "ðŸ“…",
    msg: (c) =>
`OlÃ¡ ${c.name.split(" ")[0]}! ðŸ‘‹

A sua reuniÃ£o com o escritÃ³rio Bono & Lacerda foi confirmada. ðŸŽ‰
...
ðŸ“ž +351 21 793 1934`
  ...
  {
    id: "custom",
    label: "âœï¸ Mensagem Personalizada",
    icon: "âœï¸",
    msg: () => ""
  },
];
...
function WhatsAppNotify({ client, proc, showToast }) {
  ...
  const sendWhatsApp = () => {
    ...
    if (!num) { showToast("âš ï¸ NÃºmero de telefone em falta!"); return; }
    const url = `https://wa.me/${num}?text=${encodeURIComponent(msg)}`;
    window.open(url, "_blank");
    setOpen(false);
    showToast("âœ… WhatsApp aberto!");
  };
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.5)",zIndex:2000,display:"flex",alignItems:"center",justifyContent:"center",padding:"1rem"}}>
          <div style={{background:"#fff",borderRadius:20,width:"100%",maxWidth:520,maxHeight:"90vh",overflow:"auto",boxShadow:"0 20px 60px rgba(0,0,0,.3)"}}>
            ...
            <div style={{background:"#25D366",padding:"1.25rem 1.5rem",borderRadius:"20px 20px 0 0",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div>
                <div style={{color:"#fff",fontWeight:700,fontSize:"1rem"}}>ðŸ’¬ NotificaÃ§Ã£o WhatsApp</div>
...
function ContactCard({client, setClients, showToast}){
  const fields = [
    {key:"email",    label:"ðŸ“§ Email",         type:"email"},
    {key:"phone",    label:"ðŸ“ž Telefone",      type:"tel"},
    {key:"whatsapp", label:"ðŸ’¬ WhatsApp",      type:"tel"},
    {key:"address",  label:"ðŸ  Morada",        type:"text"},
    {key:"city",     label:"ðŸ™ï¸ Cidade",        type:"text"},
    {key:"state",    label:"ðŸ“ RegiÃ£o",        type:"text"},
    {key:"zip",      label:"ðŸ“® CÃ³digo Postal", type:"text"},
    {key:"country",  label:"ðŸŒ PaÃ­s",          type:"text"},
  ];
  ...
  const save = async () => {
    ...
    if(r && r[0]){
      ...
      showToast("âœ… Dados de contacto actualizados!");
      setEditing(false);
    } else {
  ...
  return (
    <div className="card cp" style={{marginBottom:"1.25rem"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:".75rem"}}>
        <div className="ct" style={{margin:0,fontSize:"1rem"}}>ðŸ“‡ Dados de Contacto</div>
        {!editing
          ? <button className="btn btn-gh" style={{fontSize:".75rem",padding:".35rem .8rem"}} onClick={()=>setEditing(true)}>âœï¸ Editar</button>
          : <div style={{display:"flex",gap:".5rem"}}>
              ...
              <button className="btn btn-dk" style={{fontSize:".75rem",padding:".35rem .8rem"}} onClick={save} disabled={saving}>{saving?"A guardarâ€¦":"ðŸ’¾ Guardar"}</button>
            </div>
        }
      </div>
...
function Detail({cid,clients,setClients,showToast,onBack}){
  ...
  const addMeeting=async()=>{
    ...
    if(r[0]){
      setMeets(m=>[...m,r[0]]);
      await api.post("notifications",{client_id:client.id,text:`ReuniÃ£o agendada para ${mf.date.split("-").reverse().join("/")} Ã s ${mf.time}. Tipo: ${mf.type}.`,icon:"ðŸ“…",read:false});
  ...
  const confirmMeet=async m=>{
    showToast("â³ A confirmar reuniÃ£o e criar Google Meetâ€¦");
    ...
    // 2. NotificaÃ§Ã£o inicial ao cliente
    await api.post("notifications",{client_id:client.id,text:`ReuniÃ£o confirmada para ${m.date.split("-").reverse().join("/")} Ã s ${m.time}. Link da videochamada a ser geradoâ€¦`,icon:"âœ…",read:false});
    ...
    try{
      ...
      if(r.ok && data.meet_link){
        ...
        showToast("âœ… ReuniÃ£o confirmada + Google Meet criado!");
      }else{
        console.error("[confirmMeet] Meet creation failed:",data);
        showToast(`âš ï¸ ReuniÃ£o confirmada, mas falha ao criar Meet: ${data.error||"erro desconhecido"}`);
      }
    }catch(e){
      console.error("[confirmMeet] Edge function error:",e);
      showToast("âš ï¸ ReuniÃ£o confirmada, mas erro de rede ao criar Meet.");
    }
  };
  ...
  const notify=async()=>{
    await api.post("notifications",{client_id:client.id,text:"Nova atualizaÃ§Ã£o no seu processo. Acesse o portal para ver.",icon:"ðŸ””",read:false});
      ...
      <div className="card cp" style={{marginBottom:"1.25rem",display:"flex",gap:"2rem",flexWrap:"wrap",alignItems:"center"}}>
        ...
        {client.pendencias&&(
          <div style={{background:"#fef3c7",border:"1px solid #fcd34d",borderRadius:8,padding:".6rem .9rem"}}>
            <div style={{fontWeight:600,fontSize:".78rem",color:"#92400e"}}>âš ï¸ {client.pendencias}</div>
        ...
        <div className="card cp">
          ...
          {docs.map(d=>(
            <div className="dr" key={d.id}>
              ...
              <button className="ib" style={{marginLeft:4,color:"#ef4444"}} onClick={()=>delDoc(d)} title="Eliminar">ðŸ—‘ï¸</button>
            </div>
          ))}
        ...
        <div className="card cp">
          ...
          {pendentes.length>0&&(
            <div style={{background:"#fef3c7",border:"1px solid #fcd34d",borderRadius:10,padding:".9rem 1.1rem",marginBottom:"1.25rem"}}>
              <div style={{fontWeight:600,fontSize:".85rem",color:"#92400e"}}>ðŸ“¬ {pendentes.length} pedido(s) do cliente aguardando confirmaÃ§Ã£o</div>
              <div style={{fontSize:".78rem",color:"#b45309",marginTop:2}}>Clique em "âœ“ Confirmar" para criar o evento no Google Calendar automaticamente</div>
            </div>
          )}
            ...
            return(
              <div className="mcard" key={m.id} style={{borderColor:isPending?"#fcd34d":"var(--bo)",background:isPending?"#fffbf0":"var(--bg)"}}>
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{fontSize:".78rem",color:"var(--mu)",marginTop:3}}>â° {m.time} Â· {m.type==="videochamada"?"ðŸ“¹ Video":m.type==="whatsapp"?"ðŸ’¬ WhatsApp":m.type==="presencial"?"ðŸ“ Presencial":"ðŸ“ž Tel"}</div>
                  {m.notes&&<div style={{fontSize:".78rem",color:"var(--mu)",marginTop:4}}>ðŸ“ {m.notes}</div>}
                </div>
                <div style={{display:"flex",gap:".5rem",alignItems:"center"}}>
                  {isPending?(
                    <>
                      <button className="btn btn-ok" style={{fontSize:".78rem",padding:".4rem .9rem"}} onClick={()=>confirmMeet(m)}>âœ“ Confirmar + ðŸ“…</button>
                    ...
                    <>
                      <span className={`bd${m.status==="confirmado"?" bg":m.status==="recusado"?" br":" bgr"}`}>{m.status==="confirmado"?"âœ“ Confirmado":m.status==="recusado"?"Recusado":m.status}</span>
        ...
        <div className="ov" onClick={e=>e.target===e.currentTarget&&setShowMtg(false)}>
          <div className="mo">
            ...
            <div className="mob">
              ...
              <div className="ff"><label>Tipo</label>
                <select value={mf.type} onChange={e=>setMf(f=>({...f,type:e.target.value}))}>
                  ...
                  <option value="videochamada">ðŸ“¹ Videochamada</option>
            ...
            <div className="mof">
              ...
              <button className="btn btn-dk" onClick={addMeeting} disabled={saving}>{saving?<><Icon name="spin" size={15}/>Agendandoâ€¦</>:<><Icon name="check" size={15}/>Agendar + ðŸ“…</>}</button>
            </div>
          </div>
        </div>
      )}
...
function AllMeetings({clients,openClient}){
  ...
  return(
    <div>
      ...
      {pendentes.length>0&&<div style={{background:"#fef3c7",border:"1px solid #fcd34d",borderRadius:12,padding:"1rem 1.25rem",marginBottom:"1.25rem"}}><div style={{fontWeight:600,fontSize:".9rem",color:"#92400e"}}>ðŸ“¬ {pendentes.length} pedido(s) aguardando â€” abra o cliente para confirmar</div></div>}
      <div className="card cp">
        ...
        {!loading&&allMeets.map(m=>{const d=new Date((m.date||"")+"T12:00:00");return(
          <div className="mcard" key={m.id} style={{borderColor:m.status==="pendente"?"#fcd34d":"var(--bo)",background:m.status==="pendente"?"#fffbf0":"var(--bg)"}}>
            ...
            <div style={{flex:1}}>
              <div style={{fontWeight:600,fontSize:".9rem"}}>{m.title}</div>
              <div style={{fontSize:".78rem",color:"var(--mu)",marginTop:3}}>ðŸ‘¤ {m.clientId?<strong onClick={()=>openClient&&openClient(m.clientId)} style={{cursor:"pointer",color:"#2563eb",textDecoration:"underline"}}>{m.clientName}</strong>:<strong>{m.clientName}</strong>} Â· â° {m.time} Â· {m.type==="videochamada"?"ðŸ“¹":m.type==="whatsapp"?"ðŸ’¬":m.type==="presencial"?"ðŸ“":"ðŸ“ž"} {m.type}</div>
              {m.notes&&<div style={{fontSize:".78rem",color:"var(--mu)",marginTop:4}}>ðŸ“ {m.notes}</div>}
            </div>
            <span className={`bd${m.status==="confirmado"?" bg":m.status==="pendente"?" ba":" br"}`}>{m.status==="confirmado"?"âœ“ Confirmado":m.status==="pendente"?"â³ Pendente":"Recusado"}</span>
          </div>
        );})}
      </div>
    </div>
...
function AllDocuments({clients,showToast,openClient}){
  ...
  return(
    <div>
      ...
      {aguardando>0&&<div style={{background:"#fef3c7",border:"1px solid #fcd34d",borderRadius:12,padding:"1rem 1.25rem",marginBottom:"1.25rem"}}><div style={{fontWeight:600,fontSize:".9rem",color:"#92400e"}}>ðŸ“¬ {aguardando} documento(s) enviados por clientes aguardando revisÃ£o</div></div>}
      <div style={{display:"flex",gap:".5rem",marginBottom:"1.25rem"}}>
        {["todos","cliente","advogado"].map(f=>(
          <button key={f} onClick={()=>setFilter(f)} className={`btn${filter===f?" btn-dk":" btn-gh"}`} style={{padding:".5rem 1rem",fontSize:".8rem"}}>
            {f==="todos"?"ðŸ“‹ Todos":f==="cliente"?"ðŸ‘¤ Do Cliente":"âš–ï¸ Do Advogado"}
          </button>
        ))}
      </div>
      <div className="card cp">
        ...
        {!loading&&filtered.map(d=>(
          <div key={d.id} className="mcard" style={{borderColor:d.uploaded_by==="cliente"&&d.status==="aguardando"?"#fcd34d":"var(--bo)",background:d.uploaded_by==="cliente"&&d.status==="aguardando"?"#fffbf0":"var(--bg)"}}>
            <div className="mdb" style={{background:d.uploaded_by==="cliente"?"#dbeafe":"#f0fdf4",borderColor:d.uploaded_by==="cliente"?"#93c5fd":"#86efac"}}>
              <div className="day" style={{fontSize:"1.2rem"}}>ðŸ“„</div>
            ...
            <div style={{flex:1,minWidth:0}}>
              ...
              <div style={{fontSize:".78rem",color:"var(--mu)",marginTop:3}}>ðŸ‘¤ {d.clientId?<strong onClick={()=>openClient&&openClient(d.clientId)} style={{cursor:"pointer",color:"#2563eb",textDecoration:"underline"}}>{d.clientName}</strong>:<strong>{d.clientName}</strong>} Â· ðŸ“¦ {d.size} Â· ðŸ“… {d.date}</div>
            </div>
            <span className={`bd${d.uploaded_by==="cliente"?" ba":" bg"}`}>{d.uploaded_by==="cliente"?"ðŸ‘¤ Cliente":"âš–ï¸ Advogado"}</span>
            {d.storage_path&&<button className="ib" style={{marginLeft:8}} onClick={async()=>{const url=await api.signedUrl(d.storage_path);if(url)window.open(url,"_blank");else showToast("Erro ao gerar link.");}} title="Download"><Icon name="upload" size={13}/></button>}
          </div>
        ))}
      </div>
        ...
        {!loading&&allMsgs.map(m=>(
          <div className="mcard" key={m.id} style={{cursor:"pointer"}} onClick={()=>openClient&&openClient(m.clientId)}>
            ...
            <div style={{flex:1,minWidth:0}}>
              ...
              <div style={{fontSize:".72rem",color:"var(--mu)",marginTop:3}}>ðŸ“… {fmtd(m.created_at)} Â· â° {fmtt(m.created_at)}</div>
            </div>
  ...
  useEffect(()=>{
    ...
    const ch=supabase.channel('global-notifs')
      .on('postgres_changes',{event:'INSERT',schema:'public',table:'documents',filter:'uploaded_by=eq.cliente'},(payload)=>{
        ...
        showToast(`ðŸ“„ Novo documento de ${cl?cl.name:"cliente"}: "${payload.new.name}"`);
      })
      .on('postgres_changes',{event:'INSERT',schema:'public',table:'messages',filter:'from_role=eq.client'},()=>{
        setNewMsgs(n=>n+1);
        showToast(`ðŸ’¬ Nova mensagem de cliente`);
      })
        ...
        <aside className="sb">
          ...
          <div className="sbw"><div className="av" style={{width:36,height:36,fontSize:".78rem"}}>RL</div>
            <div>
              ...
              <div className="wr">OAB/PB 19.165 Â· ðŸ‡µðŸ‡¹ Lisboa 65899L Â· ðŸ‡ªðŸ‡¸ Madrid 142952</div>
            </div>
          </div>
          <nav className="sbnv">
            {nav.map(n=>(
