...
function Toast({msg,onClose}){useEffect(()=>{const t=setTimeout(onClose,3500);return()=>clearTimeout(t)},[]);return<div className="toast">v {msg}</div>;}
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.5)",zIndex:2000,display:"flex",alignItems:"center",justifyContent:"center",padding:"1rem"}}>
      <div style={{background:"#fff",borderRadius:20,width:"100%",maxWidth:520,maxHeight:"90vh",overflow:"auto",boxShadow:"0 20px 60px rgba(0,0,0,.3)"}}>
        ...
        <div style={{background:"#25D366",padding:"1.25rem 1.5rem",borderRadius:"20px 20px 0 0",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <button onClick={() => setOpen(false)} style={{background:"none",border:"none",color:"#fff",cursor:"pointer",fontSize:"1.4rem",lineHeight:1}}>x</button>
        </div>
    ...
    <div className="card cp">
      ...
      {pendentes.length>0&&(
        <div style={{background:"#fef3c7",border:"1px solid #fcd34d",borderRadius:10,padding:".9rem 1.1rem",marginBottom:"1.25rem"}}>
          ...
          <div style={{fontSize:".78rem",color:"#b45309",marginTop:2}}>Clique em "v Confirmar" para criar o evento no Google Calendar automaticamente</div>
        </div>
      )}
        ...
        return(
          <div className="mcard" key={m.id} style={{borderColor:isPending?"#fcd34d":"var(--bo)",background:isPending?"#fffbf0":"var(--bg)"}}>
            ...
            <div style={{display:"flex",gap:".5rem",alignItems:"center"}}>
              {isPending?(
                <>
                  <button className="btn btn-ok" style={{fontSize:".78rem",padding:".4rem .9rem"}} onClick={()=>confirmMeet(m)}>v Confirmar + 📅</button>
                ...
                <>
                  <span className={`bd${m.status==="confirmado"?" bg":m.status==="recusado"?" br":" bgr"}`}>{m.status==="confirmado"?"v Confirmado":m.status==="recusado"?"Recusado":m.status}</span>
...
<span className={`bd${m.status==="confirmado"?" bg":m.status==="pendente"?" ba":" br"}`}>{m.status==="confirmado"?"v Confirmado":m.status==="pendente"?"⏳ Pendente":"Recusado"}</span>
</div>
);})}
</div>
</div>
...
load(true);
const ch=supabase.channel('all-messages')
...
setNewDocs(n=>n+1);
const cl=clients.find(c=>c.proc&&c.proc.id===payload.new?.process_id);
